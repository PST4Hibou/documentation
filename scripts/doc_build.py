...
from doc import write_module_doc
...
def generate(base_path: str, docs_dir: str) -> bool:
    ...
    modules = find_all_python_modules(import_root)
    ...
    success_count = 0
    for module_name in modules:
        if generate_docs_for_module(module_name, out_dir, import_root):
            success_count += 1

    write_index(out_dir, modules, readme)

    print(f"\n🎉 Generated {success_count}/{len(modules)} modules in {out_dir}")
